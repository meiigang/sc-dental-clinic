...
type SaleEntry = {
  ...
  service: string;
  price: number;
  status: 'completed'; // We only deal with completed sales now.
};

const formatDisplayTime = (appt: SaleEntry) => {
...
const statusClass = (status: SaleEntry['status']) => {
  switch (status) {
    case "completed":
...
export default function SalesPage() {
  ...
  const chartData = useMemo(() => {
    ...
    filteredSales.forEach((sale) => {
      ...
      grouped.set(key, (grouped.get(key) || 0) + sale.price);
    });
  ...
  const totalAmount = useMemo(
    () => filteredSales.reduce((sum, a) => sum + a.price, 0),
    [filteredSales]
  );

  const totalPages = Math.max(1, Math.ceil(filteredSales.length / rowsPerPage));
  const pagedData = filteredSales.slice((page - 1) * rowsPerPage, page * rowsPerPage);
              ...
              {(() => {
                ...
                if (activeTab === "Weekly") {
                  sales.forEach((sale) => {
                    ...
                    const label = `${monday.toLocaleDateString("en-US", { month: "long", day: "numeric" })} – ${friday.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}`;
                    addIfNotExists(label, monday);
                  });
                  const { monday: thisMon, friday: thisFri } = getWeekRange(now);
                  const thisWeekLabel = `${thisMon.toLocaleDateString("en-US", { month: "long", day: "numeric" })} – ${thisFri.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })} (This Week)`;
                  ...
                  const thisMonthLabel = now.toLocaleString("default", { month: "long", year: "numeric" }) + " (This Month)";
                  addIfNotExists(thisMonthLabel, new Date(now.getFullYear(), now.getMonth(), 1), true);
                } else {
                  sales.forEach((sale) => {
                    const d = new Date(sale.date);
                ...
                const filteredItems = thisItem ? [thisItem, ...items.filter((i) => i !== thisItem)] : items;

                return filteredItems.map(({ label, date, isThis }, idx) => (
                  <DropdownMenuItem key={idx} onClick={() => setCurrentDate(date)} className={isThis ? "bg-blue-50 text-blue-700 font-medium" : "hover:bg-blue-100"}>
                    {label}
                  </DropdownMenuItem>
                ));
              })()}
              ...
              {pagedData.length > 0 ? (
                pagedData.map((sale) => (
                  <tr key={sale.id} className="text-center bg-white hover:bg-blue-100 transition-colors">
                    ...
                    <td className="p-3 border border-blue-accent">{sale.service}</td>
                    <td className="p-3 border border-blue-accent">₱{sale.price.toLocaleString()}</td>
                    <td className="p-3 border border-blue-accent">
                      <span className={statusClass(sale.status)}>Completed</span>
                    </td>
                  </tr>
                ))
              ) : (
        ...
        <div className="mt-12 w-full max-w-6xl">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-2">
            ...
            <p className="text-gray-500 text-sm mt-2 sm:mt-0">
              Based on {filteredSales.length} completed appointment{filteredSales.length !== 1 ? "s" : ""}.
            </p>
          </div>
