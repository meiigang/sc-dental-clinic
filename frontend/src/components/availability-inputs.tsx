      ...
      <div>
        ...
        <div className="flex flex-col gap-2 mt-4">
          {days.map((day, dIdx) => (
            ...
            >
              {/* Day label */}
              <span className="bg-blue-accent p-2 w-16 min-w-16 max-w-16 h-8 flex-shrink-0 flex items-center justify-center rounded-lg font-medium text-blue-dark text-base select-none">
                {getDayLabel(day.key)}
              </span>
          ...
          {dateSpecific
            ...
            .map((ds, idx) => (
              <div key={idx} className="flex items-center gap-10 bg-muted rounded-lg px-4 py-2">
                <span className="text-sm min-w-[80px]">{formatDate(ds.date)}</span>
